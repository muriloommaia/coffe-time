...
export default function ProgressBar() {
  const { escrita, compartir, lectura } = useSelector((state: RootState) => state);

  const calcPercentage = (time: number, total: number) => Math.abs(Math.ceil(((time / total) * 100) - 100));

  const progress = () => {
    const total = escrita.time + compartir.time + lectura.time;
    const left = escrita.timeLeft + compartir.timeLeft + lectura.timeLeft;
